...
The plugin exposes nine skills:
...
8. **`ricontrolla-ce-a105`** — targeted, on-demand audit across the entire `Output/` archive: for every Heat sheet with Grado Materiale A105, checks that the chemical field CE (Carbon Equivalent) is filled in. If it's blank, it doesn't accept that as a normal missing field — it re-reads the already-archived original certificate more thoroughly (alternate field names such as CEV, Exp.4, CEQ, C EQ. LONG FORMULA; a higher-resolution re-scan via `prepara-pdf-per-ocr` if needed), and regenerates the sheet with the value found, overwriting the previous file in `Output/`. If CE genuinely can't be found even after that deeper re-read, it flags the Heat for manual verification instead of silently leaving it blank. Not part of the automatic per-PDF flow — run it on request.
9. **`esperto-materiali-metallurgici`** — reference on chemical and mechanical limits for the material grades LAME receives most often (SA-105/A105N, SA-182 F5, F11 Cl.1/Cl.2, F316/F316L, F53, SA-350 LF2 Cl.1, A694 F52): compares the chemical composition and mechanical properties already extracted by `quality-steel` (or read directly from a certificate) against the applicable spec limits and flags any out-of-range value or missing required test (e.g. a missing impact test on an LF2 Cl.1). Reference/validation only — it never edits `Output/Scheda_Heat_*.pdf` on its own initiative.
...
- "Recheck the CE on A105 materials" / "check the carbon equivalent is filled in on A105 sheets" → `ricontrolla-ce-a105`
- "Is this A182 F11 Cl.2 within spec?" / "check the chemical/mechanical limits for this grade" / "any out-of-spec heats in what's already archived?" → `esperto-materiali-metallurgici`
